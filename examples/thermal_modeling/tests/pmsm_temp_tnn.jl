...
function main()
    ...
    while i * tbptt_len <= max_len_train
        push!(
            train_vec_chunked_x,
            train_vec_x[i*tbptt_len+1:minimum(((i + 1) * tbptt_len + 1, max_len_train))],
        )
        push!(
            train_vec_chunked_y,
            train_vec_y[i*tbptt_len+1:minimum(((i + 1) * tbptt_len + 1, max_len_train))],
        )
        push!(
            train_vec_chunked_w,
    ...
    test_vec_temps_x = [test_tensor_temps_x[i, :, :] for i = 1:size(test_tensor_temps_x, 1)]
    test_vec_non_temp_x =
    ...
    m = Flux.Recur(
        TNNCell(
            length(c_non_temps) + length(c_input_temps),
            length(c_temps),
            length(target_cols),
            init_hidden,
        ),
